...
class UserAccountSettingsView: UIView {
  ...
  private struct Constants {
    ...
    static let interItemVerticalSpacing: CGFloat = 24
  ...
  init(_ viewModel:SettingsViewModel) {
    ...
    eventTimesSwitch.isOn = viewModel.isEventsInPacificTime
    notificationsSwitch.isOn = viewModel.isNotificationsEnabled
    analyticsSwitch.isOn = viewModel.isAnalyticsEnabled
  }

  override var intrinsicContentSize: CGSize {
    return CGSize(width: UIViewNoIntrinsicMetric, height: UserAccountSettingsView.heightForContents())
  }
  ...
  static func heightForContents() -> CGFloat {
    let textHeights = [
      ...
      return result + string.boundingRect(with: CGSize(width: CGFloat.greatestFiniteMagnitude,
                                          ...
                                          attributes: [NSAttributedStringKey.font: Constants.labelFont()],
  ...
  @objc func didTapSwitch(_ anySender: Any) {
    ...
    // TODO(morganchen): This class only writes the permissions, others must read them and make use.
...
class UserAccountSettingsBuiltWithView: UIView {
  ...
  let builtWith: UILabel = {
    ...
    builtWith.font = MDCTypography.subheadFont()
  ...
  let materialComponents: UILabel = {
    ...
    materialComponents.font = MDCTypography.headlineFont()
  ...
  override var intrinsicContentSize: CGSize {
    return CGSize(width: UIViewNoIntrinsicMetric, height: UserAccountSettingsBuiltWithView.heightForContents())
  }
}

protocol UserAccountSettingsInfoViewDelegate {
  func didTapOpenSourceLicenses(view: UIView)
}

class UserAccountSettingsInfoView: UIView {

  var delegate: UserAccountSettingsInfoViewDelegate?
  ...
  fileprivate struct Constants {
    ...
    // TODO(morganchen): check if we have an official URL scheme.
  ...
  private func setupTextView() {
    ...
    textView.linkTextAttributes = [NSAttributedStringKey.foregroundColor.rawValue: Constants.linkColor]
